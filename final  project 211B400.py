...
class bus_booking:
    ...
    def cover_page(self):
        ...
        Label(root, text="\n\n\n\n\nName: Mohan Choudhary", font='Arial 12 bold', fg='blue').pack()
        Label(root, text="\nEnr : 211B400", font='Arial 12 bold', fg='blue').pack()
                ...
                if not jd=='':
                    ...
                    if len(res_route)==0:
                        ...
                        if len(res_bid)==0:
                            ...
                            if len(b)==0:
                                showerror('No Bus Available',"Enter Another date!!")
                            else:
                                ...
                                def proceed():
                                    ...
                                    Label(root,text='name',font='Arial 10 bold').grid(row=12,column=3)
                                    pname = Entry(root, font='Arial 12 bold', fg='black')
                                    pname.grid(row=12,column=4)

                                    gender = StringVar()
                                    gender.set("Select Gender")
                                    opt = ["Male", "Female", "Other"]
                                    g_menu = OptionMenu(root, gender, *opt)
                                    g_menu.grid(row=12, column=6)

                                    Label(root, text='No of seats', font='Arial 10 bold').grid(row=12, column=7)
                                    pseat=Entry(root, font='Arial 12 bold', fg='black')
                                    pseat.grid(row=12,column=8)

                                    Label(root, text='Mobile', font='Arial 10 bold').grid(row=13, column=3)
                                    pmobile = Entry(root, font='Arial 12 bold', fg='black')
                                    pmobile.grid(row=13, column=4)

                                    Label(root, text='Age', font='Arial 10 bold').grid(row=13, column=5)
                                    page = Entry(root, font='Arial 12 bold', fg='black')
                                    page.grid(row=13, column=6)
                                                ...
                                                if age>0 and age<150:
                                                    if seats>0 and seats<100:
                                                        ...
                                                        if seats <= s:
                                                            ...
                                                            showerror("seats not available",'enter valid seat')

                                                    else:
                                                        showerror("Booking limit exceed","you can only book upto 10 seats")
                                                else:
                                    ...
                                    Button(root, text='Book Seat', bg='green', fg='black', font='Arial 12 bold',
                                           command=book_seat).grid(row=13, column=8, padx=30)



                else:
                    showerror('error','Enter date')
    ...
    def check_booking_page(self):
        ...
        def check_tkt():
            ...
            if len(mobile)==10 and mobile.isdigit():
                ...
                Label(text="Booked Ticket Details", font='Arial 12 bold',fg='red',bg='light grey').grid(row=6,columnspan=10 )
                Label(text="Booking ref = "+b_ref,font='Arial 12 bold', fg='black').grid(row=8,column=4)
                Label(text="Name = " + name, font='Arial 12 bold', fg='black').grid(row=9, column=4)
                Label(text="Gender = " + gen, font='Arial 12 bold', fg='black').grid(row=10, column=4)
                Label(text="No of seats = " + str(seat), font='Arial 12 bold', fg='black').grid(row=8, column=6)
                Label(text="Age = " + str(age), font='Arial 12 bold', fg='black').grid(row=8, column=5)
                Label(text="Booked on = " + book_date, font='Arial 12 bold', fg='black').grid(row=9, column=6)
                Label(text="Travel date = " + travel_date, font='Arial 12 bold', fg='black').grid(row=10, column=5)
                Label(text="Fare = " + str(fare), font='Arial 12 bold', fg='black').grid(row=9, column=5)
                Label(text="Total Fare = " + str(fare*seat), font='Arial 12 bold', fg='black').grid(row=10, column=6)




        Label(root, image=bus).grid(row=0, column=0, columnspan=12, padx=80)
        Label(root, text="Online Bus Booking System", font='Arial 28 bold', bg='light blue', fg='red').grid(row=2,column=0,columnspan=12,pady=20)
        Label(root, text="Check Your Booking", font='Arial 22 bold', bg='pale green', fg='white').grid(row=3,column=0,pady=20,columnspan=12,padx=600)

        Label(root, text="Mobile No.", font='Arial 12 bold', fg='black').grid(row=4, column=5)
    ...
    def add_bus_op_details_page(self):
        ...
        def op_add():
            ...
            if len(iid) > 0 and len(iid) <= 5 and iid.isnumeric():
                if  len(iname) < 40 and len(iname) > 0:
                    if len(iaddress) < 100 and len(iaddress) > 0:
                        if iphone.isnumeric() and len(iphone) == 10:
                            if len(iemail) > 0 and len(iemail) < 35:
                                if (iid,) in res:
    ...
    def add_bus_page(self):
        ...
        opt = ["3*2", "2x2", "AC 3x2", "AC 2x2","Non-AC"]
    ...
    def add_route_page(self):
        ...
        Label(root, text="From station", font='Arial 12 bold', fg='black').grid(row=3, column=2)
        s_station=Entry(root, font="Arial 12")
        s_station.grid(row=3, column=3, padx=50)

        Label(root, text="Station Id1", font='Arial 12 bold', fg='black').grid(row=3, column=4)
        s_id=Entry(root, font="Arial 12")
        s_id.grid(row=3, column=5, padx=50)

        Label(root, text="To station", font='Arial 12 bold', fg='black').grid(row=4, column=2)
        e_station=Entry(root, font="Arial 12")
        e_station.grid(row=4, column=3, padx=50)

        Label(root, text="Station Id2", font='Arial 12 bold', fg='black').grid(row=4, column=4)
